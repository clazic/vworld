...
- **★ 저장소 파일명에 `:` 금지(Windows 러너가 죽는다)** — 파일명에 콜론이 있으면 windows-latest의 `actions/checkout`이 `invalid path` → exit 128로 실패하고, 3-OS 매트릭스 중 하나만 죽어도 release job이 `skipped`되어 **릴리스 자산이 하나도 안 올라간다**(v0.2.1 실측). 계획서 시각 구분자는 하이픈으로(`2026-07-16-12-17-49-제목.md`). `: * ? " < > |` 전부 회피. 이미 커밋됐으면 `git mv` 후 태그를 다시 찍어야 릴리스가 복구된다.
- **릴리스 자산은 SHA256SUMS로 검증** — `sha256sum <파일들> | sed 's| [^ ]*/| |'`로 **경로를 벗겨 평탄한 파일명**만 남겨야 `update`가 자산명으로 조회할 수 있다. 구 릴리스에는 이 파일이 없으므로 404는 실패가 아니라 "검증 생략" 경고로 처리(롤백 경로 보존), 파일이 있는데 해시가 다르면 즉시 중단.
...
- 2026-07-20 | 릴리스 CI(v0.2.1) | windows 빌드가 checkout에서 `invalid path 'plan/2026-07-16-12:17:49-...md'` exit 128 → release job skipped, **자산 0개 게시** | Windows는 파일명에 `:`를 허용하지 않는데 계획서 형식이 콜론을 쓰고 있었음. Rust 코드는 무결 — 실패 지점이 코드가 아니라 체크아웃이라 로그를 안 보면 원인 오판하기 쉬움 | `git mv`로 콜론 제거 후 **태그 삭제·재푸시**(태그는 커밋을 가리키므로 수정 커밋만으로는 재빌드 안 됨). 일반 규칙으로 승격.
- 2026-07-20 | clap 도움말 | 여러 줄 doc comment로 KIND 11종을 목록으로 적었는데 `--help`에서 **한 문단으로 뭉개져** 출력 | clap derive는 doc comment의 줄바꿈을 문단 내 개행으로 보지 않고 합쳐서 재래핑함(빈 줄만 문단 구분) | 목록·정렬이 필요한 설명은 `#[arg(long_help = "...\n...")]` 문자열로 주면 개행이 보존된다. 한 줄 요약은 doc comment에 남겨 `-h`용으로 유지.
- 2026-07-20 | clap 도움말 위치 | `GeocodeArgs` 등 구조체 위에 쓴 long_about 문단이 `--help`에 **안 나옴** | 이 구조체들은 `#[command(flatten)]`으로 합쳐지므로 명령 설명은 `cli/mod.rs`의 `Commands` enum variant doc이 이긴다 | 명령 단위 설명은 `Commands` variant에, 인자 단위 설명은 필드 doc에 쓴다. 구조체 doc은 rustdoc 전용으로만 남음.
- 2026-07-20 | self-update 재작성 | `self_update` crate로는 sgis식 한글 프롬프트·단계별 출력을 만들 수 없음(영문 진행바·자체 확인 프롬프트가 crate 내부 고정) | 출력과 프롬프트 시점이 crate에 하드코딩 | 최신태그→SHA256SUMS→다운로드→확인→검증→교체→스킬갱신 파이프라인을 직접 구현(reqwest+sha2+zip). 부수효과로 ureq·indicatif·reqwest 중복 의존이 사라짐.
- 2026-07-20 | GitHub API 호출 | UA 없이 `releases/latest` 요청하면 **403**, UA 붙이면 200(실측: `curl -H "User-Agent:"` → 403 / `curl -A "vworld/x"` → 200) | GitHub API는 User-Agent 없는 요청을 거부. reqwest는 Go 표준 클라이언트와 달리 UA를 자동으로 붙이지 않음 | 클라이언트 빌더에 `.user_agent(concat!("vworld/", env!("CARGO_PKG_VERSION")))` 필수. 리다이렉트는 reqwest 기본(최대 10회)으로 릴리스 다운로드에 충분.
- 2026-07-20 | reqwest 응답 파싱 | `resp.json()` 컴파일 에러(`no method named json`) | Cargo.toml에서 `default-features = false`로 쓰고 있어 **`json` feature가 꺼져 있음** | feature를 켜기보다 `resp.text()` + `serde_json::from_str`로 처리(의존 표면 최소). 이미 serde_json은 들어와 있음.
- 2026-07-20 | 바이너리 자기교체 | 임시파일을 `os.tmpdir()`에 만들고 rename하면 파일시스템이 다를 때 EXDEV로 실패 | rename은 같은 파일시스템 안에서만 원자적 | 임시파일을 **교체 대상과 같은 디렉터리**에 `dst.new`로 만들고 rename. Windows는 실행 중 exe를 지울 수 없어 `dst→dst.old` 선이동 후 교체. 실패 시 임시파일 정리.
- 2026-07-20 | update 직후 알림 | 교체 성공 직후 "새 버전 vX가 있습니다" 알림이 또 출력 | 교체를 수행한 프로세스는 **여전히 구버전**이라 종료 직전 하루1회 감지가 방금 설치한 버전을 새 버전으로 안내 | `update` 명령 실행 시 알림 억제 플래그를 세움. 다른 명령에서는 정상 동작하는지 함께 확인할 것.
- 2026-07-20 | 스킬 자산 분리 | 업데이트마다 `vworld-skill.zip`을 받으면 **OS별 바이너리 4개(100MB+)** 를 매번 내려받게 됨 | 설치용 zip은 자기완결을 위해 바이너리를 품고 있음 | 업데이트 전용 `vworld-skill-files.zip`(문서·references만) 자산을 분리 신설. 설치 스크립트가 의존하는 기존 zip은 이름·내용 그대로 유지(호환).
- 2026-07-20 | 스킬 갱신 범위 | 업데이트 zip에 `LEARNINGS.md`를 넣으면 사용자가 append한 오답노트가 **덮어써짐** | 이 파일은 자기학습 누적 기록이라 배포본으로 덮으면 안 됨 | 업데이트 번들(`vworld-skill-files.zip`) 화이트리스트를 `SKILL.md·INSTALL.md·README.md·references/`로 한정. `LEARNINGS.md`·`scripts/`는 갱신 대상에서 제외(실측 확인).
- 2026-07-20 | update 권한 실패 | 읽기 전용 설치 디렉터리에서 교체 시 안내 없는 `Permission denied (os error 13)`만 노출 | 실패 지점이 rename이 아니라 **그 앞의 임시파일(`dst.new`) 쓰기** 였다 — 해결 안내를 rename 쪽에만 달아둬서 정작 가장 흔한 경로에서 안 나왔다 | 쓰기 단계에서 `ErrorKind::PermissionDenied`를 구분해 "sudo 또는 설치 스크립트 재실행" 안내로 치환. 종료코드 1·기존 바이너리 보존은 정상 확인. 회귀 테스트 추가(`test_replace_binary_permission_denied`). **에러 안내는 실제로 먼저 터지는 지점에 달아야 한다.**
- 2026-07-20 | 설치 zip 누락 | `vworld-skill.zip`에 **`LEARNINGS.md`·`README.md`가 아예 없음**(INSTALL.md·SKILL.md만) — 그런데 SKILL.md는 "작업 전 LEARNINGS.md를 먼저 읽어라"라고 지시 | 설치 번들 구성에서 두 파일이 빠져 있었음. git 복사본으로 설치한 환경에는 있어서 눈에 안 띄었다 | **설치용 zip에는 넣고(1회 시딩), 업데이트용 zip에서는 뺀다**(덮어쓰기 방지). 두 번들의 역할이 다르다는 점을 구성 시 항상 구분할 것.
- 2026-07-20 | geocode 역지오 유형 | 기존 노트의 "역지오는 항상 `--type BOTH`"를 재검증 | 현재 CLI는 `--type auto`(기본)가 역지오에서도 ROAD→PARCEL 폴백을 돌려 결과가 나옴. `BOTH`는 CLI가 그대로 전달하는 **서버 지원 값**으로 지번+도로명을 한 번에 반환 | 기본 auto로 충분하고, 지번·도로명을 **동시에** 받고 싶을 때만 `--reverse --type BOTH`. 도움말에도 명시함.
- 2026-07-20 | 테스트 스크립트(zsh) | `for c in "" "data layers"; do $B $c --help; done`이 "unrecognized subcommand 'data layers'"로 전부 실패 | **zsh는 unquoted 변수 확장 시 단어 분리를 하지 않는다**(bash와 다름) — 인자 두 개가 한 덩어리로 전달됨 | zsh에서는 `${=c}`(또는 배열)로 명시적 분리. 이걸 모르면 멀쩡한 CLI를 버그로 오진한다.
