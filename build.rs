//! NED 레지스트리 codegen — `skills/data/ned_catalog.tsv`(단일 출처) → 정적 테이블 Rust 생성.
//!
//! 드리프트 가드(설계 §8-⑩): 행수 115 및 prefix 집계 36/33/46 불일치 시 빌드 실패.
//! 추가: `skills/data/ned_params.tsv` → NedParam 정적 슬라이스 코드젠.
...
fn main() {
    println!("cargo:rerun-if-changed=skills/data/ned_catalog.tsv");
    println!("cargo:rerun-if-changed=skills/data/ned_params.tsv");

    // ── 1. ned_catalog.tsv 파싱 ──────────────────────────────────────────────
    let tsv = fs::read_to_string("skills/data/ned_catalog.tsv")
        .expect("skills/data/ned_catalog.tsv 읽기 실패");
    ...
    let params_tsv = fs::read_to_string("skills/data/ned_params.tsv")
        .expect("skills/data/ned_params.tsv 읽기 실패");
    ...
    out.push_str("// @generated by build.rs from skills/data/ned_catalog.tsv + ned_params.tsv — 수정 금지.\n");
...
/// 2D 데이터 158 레이어 코드젠.
/// `skills/data/twod_catalog.tsv` + `skills/data/twod_attrs.tsv` →
/// `OUT_DIR/twod_catalog_gen.rs`
fn gen_twod_catalog(out_dir: &str) {
    println!("cargo:rerun-if-changed=skills/data/twod_catalog.tsv");
    println!("cargo:rerun-if-changed=skills/data/twod_attrs.tsv");

    // ── 1. twod_catalog.tsv 파싱 ──────────────────────────────────────────
    // 형식: 헤더 + N행, 컬럼 data_id\tsvc_ide\tname\tcat\tgeom
    let cat_tsv = fs::read_to_string(Path::new("skills/data/twod_catalog.tsv"))
        .expect("skills/data/twod_catalog.tsv 읽기 실패");
    ...
    let attrs_tsv = fs::read_to_string(Path::new("skills/data/twod_attrs.tsv"))
        .expect("skills/data/twod_attrs.tsv 읽기 실패");
    ...
    let mut out = String::new();
    out.push_str(
        "// @generated by build.rs from skills/data/twod_catalog.tsv + twod_attrs.tsv — 수정 금지.\n",
    );
